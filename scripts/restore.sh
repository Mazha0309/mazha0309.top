...
"${compose[@]}" exec -T postgres psql \
  ...
  --dbname "$postgres_db" \
  --set ON_ERROR_STOP=1 \
  --command "truncate table public.comments, public.comment_settings, public.post_revisions, public.post_slugs, public.posts, public.projects, public.friend_links, public.pages, public.music_tracks, public.content_links, public.site_profiles, public.media cascade"
